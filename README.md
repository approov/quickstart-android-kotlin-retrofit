...
* A solid understanding of how to integrate Approov into your own app that uses Kotlin and [Retrofit](https://square.github.io/retrofit/)
...
The folder `framework` has a `src/main/java/io/approov/framework/retrofit/ApproovService.kt` file. This provides a wrapper around the Approov SDK itself to make it easier to use with `Retrofit`. Show the terminal in Android Studio by clicking on the `Terminal` tab, typically located at the bottom of the screen:
...
This initializes Approov when the app is first created. It uses the configuration string we set earlier. The member allows other parts of the app to access the singleton Approov instance. All calls to `ApproovService` and the SDK itself are thread safe.
...
val retrofit = retrofit2.Retrofit.Builder().baseUrl("my.domain/").client(client).build()
```
Pass the modified builder to the `ApproovService` framework as follows:
```
<AppClass>.approovService!!.setOkHttpClientBuilder(OkHttpClient.Builder().connectTimeout(5, TimeUnit.SECONDS))
val retrofitBuilder = retrofit2.Retrofit.Builder().baseUrl("my.domain/")
val retrofit = <AppClass>.approovService!!.getRetrofit(retrofitBuilder)
```
